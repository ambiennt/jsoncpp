    ...
    */
    ...
    Value& ValueInternalMap::resolveReference(const char* key, bool isStatic) {
        ...
        if (bucketsSize_) {
            ...
            BucketIndex index;
            for (ValueInternalLink* current = &buckets_[bucketIndex]; current != 0; previous = &current->next_, current = current->next_) {
    ...
    bool ValueInternalMap::equals(const IteratorState& x, const IteratorState& other) {
        return x.map_ == other.map_ && x.bucketIndex_ == other.bucketIndex_ && x.link_ == other.link_ && x.itemIndex_ == other.itemIndex_;
    }

    void ValueInternalMap::incrementBucket(IteratorState& iterator) {
        ++iterator.bucketIndex_;
        JSONCPP_ASSERT_MESSAGE(iterator.bucketIndex_ <= iterator.map_->bucketsSize_, "ValueInternalMap::increment(): attempting to iterate beyond end.");
    ...
    void ValueInternalMap::increment(IteratorState& iterator) {
        ...
        if (iterator.itemIndex_ == ValueInternalLink::itemPerLink) {
            JSONCPP_ASSERT_MESSAGE(iterator.link_ != 0, "ValueInternalMap::increment(): attempting to iterate beyond end.");
